...
@break
	...
	@if($count%20==0)
		 <ul class="pagination">
	 @if($page>1)
	  <li><a href="{{route('user.products.motivation').'?page='.($page-1).$searchurl}}">&laquo;</a></li>@endif
			 @for ($i = 0; $i < $count/20; $i++)
				 @if($page==$i+1)
					 <li class="active"><a href="{{route('user.products.motivation').'?page='.($i+1).$searchurl}}">{{$i+1}}</a></li>
				 @else
					  <li><a href="{{route('user.products.motivation').'?page='.($i+1).$searchurl}}">{{$i+1}}</a></li>
				 @endif
    
@endfor  
	 @if($page<($count/20))
<li><a href="{{route('user.products.motivation').'?page='.($page+1).$searchurl}}">&raquo;</a></li>@endif
</ul>
@else
	 <ul class="pagination">
   @if($page>1)
