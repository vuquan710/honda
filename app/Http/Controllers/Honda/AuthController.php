...
class AuthController extends UserAppController
{
    ...
    protected $dirView = 'HondaView.Auth.';
...
//        }
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($validator->fails()) {
                return redirect()->route('user.auth.login')->withErrors($validator->errors())
                    ->withInput();
            }
            $data['deleted_at'] = null;
            $remember = isset($request['remember']) && $request['remember'] == 1 ? true : false;
            $auth = Auth::guard('users')->attempt($data, $remember);
            if (!empty($auth)) {
                if (Cache::has($request->ip())) {
                    Cache::forget($request->ip());
                }
                return redirect()->route('user.homes.index');
            } else {
                return redirect()->route('user.auth.login')->withErrors(['Login fails!'])
                    ->withInput();
            }
